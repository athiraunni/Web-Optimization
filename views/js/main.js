...
var resizePizzas = function(size) {
  ...
  //changed text for size change
  //changed function to return different sizes
  // sizes selection using getElementByID instead of querySelector
...
// moved pizzaDiv out to reduce frequency accessing DOM
...
function updatePositions() {
  ...
  // changed selector to getElementID
  var items = document.getElementsByClassName('.mover');
  // call scrollTop only once instead of each cycle through the loop
  var theScrollTop = document.body.scrollTop;
  // Moved the "phase" variable creation outside the loop
  var phase;

  for (var i = 0, len = items.length; i < len; i++) {
    // save the array length in local var : array length isn't accessed per iteration.
    phase = Math.sin((theScrollTop / 1250) + (i % 5));
    // combined 'var newPOS' and 'items[i].style' lines to 1 line with style.transform
    items[i].style.transform = 'translateX(' + 100 * phase + 'px)';
    // removed -600 op bcz it prevent pizza from moving
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Generate total pizzas required based on available screen dimensions & round up ceil to next whole number
  var pizzaTotal = Math.ceil(cols * rows);
  // Select movingPizzas using getElementByID instead of querySelector.
